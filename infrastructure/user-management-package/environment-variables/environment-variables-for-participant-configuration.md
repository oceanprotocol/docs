# Environment Variables for Participant Configuration
...
**Description:** Specifies the URL of the Marketplace server that Participant users connect to. This value must match the [MARKETPLACE\_URL](environment-variables-for-dataspace-operator-configuration.md#marketplace_url) configured in the corresponding Dataspace Operator deployment.
...
#### CENTRAL\_IDP\_WELL\_KNOWN\_URL

**Description:** Specifies the URL of the well‑known endpoint of the Authentik provider configured on the Central Identity Provider for Marketplace authentication. This endpoint is used by the Participant initialization script to retrieve the provider’s OIDC endpoints and related metadata.&#x20;

**Values:** String (URL)

**Example:** `https://ocean-node-vm1-stage.oceanenterprise.io:9443/application/o/main-oidc-app/.well-known/openid-configuration`

**Default Value:** `null`&#x20;

&#x20;

#### CENTRAL\_IDP\_CLIENT\_ID

**Description:** Specifies the Client ID of the OIDC provider configured on the Central Identity Provider for Marketplace authentication.
...
#### CENTRAL\_IDP\_CLIENT\_SECRET

**Description:** Specifies the Client secret of the OIDC provider configured on the Central Identity Provider for Marketplace authentication.

**Values:** String&#x20;

**Example:** `strongsecretkey`

**Default Value:** `null`&#x20;



#### CENTRAL\_IDP\_PROVIDER\_NAME

**Description:** Specifies the name of the OIDC provider configured on the Central Identity Provider for Marketplace authentication.
...
**Description:** Specifies the slug of the Authentik application that will be created on the Participant's Authentik server to authenticate users to the OE Marketplace. For simplicity, the Authentik application name and application slug will have the same value.&#x20;

This slug must be unique within a dataspace, as it serves as the identifier of the Participant. To this end, the Dataspace Operator might set up a naming convention in the onboarding procedure for dataspace participants. For instance, the slug can be set to `<participant_name>-app`.
...
**Description:** Specifies the name of the Authentik provider associated with the Authentik application identified by the [AUTHENTIK\_APP\_SLUG](environment-variables-for-participant-configuration.md#authentik_app_slug) variable.&#x20;

This provider name must be unique within a dataspace, as it serves as the identifier of the Participant. To this end, the Dataspace Operator might set up a naming convention in the onboarding procedure for dataspace participants. For instance, the provider name can be set to `<participant_name>-provider`.
...
Specifies the name of the blueprint file generated when the initialization script is executed. This blueprint file contains the Authentik server configuration specific to the current participant deployment and is produced based on the values of the variables defined in this chapter. The blueprint is imported automatically after the Authentik server containers have started.&#x20;

**Values:** String&#x20;

**Example:** `participant-operator-authentik-blueprint.yaml`

**Default Value:** `participant-operator-authentik-blueprint.yaml`
